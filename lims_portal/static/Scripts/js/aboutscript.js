...
function initPage() {
  ...
  // Each section takes 20% of the total scroll distance
  for (let i = 0; i < 7; i++) {
    sectionPositions.push(fixedSectionTop + (fixedSectionHeight * i) / 7);
  }
  ...
  artists.forEach((artist, index) => {
    artist.addEventListener("click", (e) => {
      if (index === 0) return; // Allow default for Home link
      e.preventDefault();
  ...
  const mainScrollTrigger = ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      const targetSection = Math.min(6, Math.floor(progress * 7));
      ...
      const sectionProgress = currentSection / 6;
